 ...
 */
public class FamilyDetailObject implements ResultSetConvertible {
    ...
    public static void getAllPensioners(Context context, Database.DataReceiver receiver) {
        currentID = 0;
        ArrayList<FamilyDetailObject> pensioners = new ArrayList();
    ...
    public static void getAll (final Context context, final Database.DataReceiver receiver,String id, String name) {
        ...
        if (!Database.isConnected())
            Database.connect(context);
        String query =  "select District_Code,Block_Town_Code,Gram_Ward_Code,R_U,Applicant_Name,Applicant_adhar,Applicant_ID,Applicant_mob,Ispregnent,fl_name," +
                "fl_age, iseducated_15, isregno_center, educenter_name, educenter_no, edu_p_name,isagricul_land, how_land," +
                "isbusiness, business_name, Isjob, job_type,job_desig,job_salary,isprsnl_home,home_land, home_type,is_home_rent," +
                "home_month_rent,ismotorcycle,mc_number,isother_mc,other_mc_type,other_mc_reg,istructor_ptiler,tab_no,f_time," +
                " family_auto_no, save_ip,ischild5,ischild614 from " + TABLE_NAME + " where Applicant_ID = '" + id + "'" + " and edu_p_name= '" + name + "'";
        ...
        Database.execQueryWithDialog(
                ...
                "select District_Code,Block_Town_Code,Gram_Ward_Code,R_U,Applicant_Name,Applicant_adhar,Applicant_ID,Applicant_mob,Ispregnent,fl_name," +
                        "fl_age, iseducated_15, isregno_center, educenter_name, educenter_no, edu_p_name,isagricul_land, how_land," +
                        "isbusiness, business_name, Isjob, job_type,job_desig,job_salary,isprsnl_home,home_land, home_type,is_home_rent," +
                        "home_month_rent,ismotorcycle,mc_number,isother_mc,other_mc_type,other_mc_reg,istructor_ptiler,tab_no,f_time," +
                        " family_auto_no, save_ip,ischild5,ischild614 from " + TABLE_NAME + " where Applicant_ID = '" + id + "'" + " and edu_p_name= '" + name + "'"
    ...
    public static void getAllAdd (final Context context, final Database.DataReceiver receiver,String id, String name) {
        currentID = 0;
        System.out.println("id" + id);
        upOrAdd="add";
        if (!Database.isConnected())
        ...
        Log.i("query get all", query);
        String query1="select District_Code,Block_Town_Code,Gram_Ward_Code,R_U, Applicant_Name,Applicant_adhar,Applicant_ID,Applicant_mob,Ispregnent,fl_name,"+
                "fl_age, iseducated_15, isregno_center, educenter_name, educenter_no, edu_p_name,isagricul_land, how_land," +
                "isbusiness, business_name, Isjob, job_type,job_desig,job_salary,isprsnl_home,home_land, home_type,is_home_rent," +
                "home_month_rent,ismotorcycle,mc_number,isother_mc,other_mc_type,other_mc_reg,istructor_ptiler,tab_no,f_time," +
    ...
    public FamilyDetailObject convert (ResultSet resultSet) {
        FamilyDetailObject pensioner = null;

        try {
            pensioner = new FamilyDetailObject(
                    FamilyDetailObject.currentID++,
                    FamilyDetailObject.upOrAdd,
    ...
    public static void update (Context context, Database.DataReceiver receiver, String id, FamilyDetailObject pensioners) {
        if (!Database.isConnected())
            Database.connect(context);
